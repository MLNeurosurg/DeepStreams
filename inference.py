...
def feedfoward(model, raw_image, transform, device='cuda'):
    ...
    pred = resize(pred, output_shape=(height, width),
                  order=3)  # resize prediction
...
def save_predictions(model,
    ...
    plt.imsave(os.path.join(preds_path, img_filename[0:-4] + "_pred.png"),
               pred,
               cmap="viridis",
               vmin=0,
               vmax=255)
    plt.imsave(os.path.join(crops_path, img_filename[0:-4] + "_crop.png"),
               img,
               vmin=0,
               vmax=255)
...
if __name__ == "__main__":
    ...
    # load model
    model = torch.load('models/unet_oncostreams_acc86.pt',
